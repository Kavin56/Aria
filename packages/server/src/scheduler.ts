...
function ensureSchedulerSupported() {
  ...
  throw new ApiError(400, "scheduler_unsupported", "Scheduler is supported on macOS, Linux, and Windows.");
}
...
function schedulerSystemPaths(job: ScheduledJob): string[] {
  ...
  if (process.platform === "win32") {
    // Windows Task Scheduler task names
    const taskName = job.scopeId ? `OpenCode_${job.scopeId}_${job.slug}` : `OpenCode_${job.slug}`;
    paths.push(taskName); // Task name for schtasks.exe
    return paths;
  }
...
async function uninstallJob(job: ScheduledJob): Promise<void> {
  ...
  if (process.platform === "win32") {
    ...
    const taskNames = schedulerSystemPaths(job);
    for (const taskName of taskNames) {
      // Use schtasks.exe to delete the task
...
export async function listScheduledJobs(workdir?: string): Promise<ScheduledJob[]> {
  // Allow listing jobs on all platforms (including Windows)
  // Installation/uninstallation may have platform-specific requirements
...
export async function resolveScheduledJob(
  ...
  // Allow resolving jobs on all platforms
  const trimmed = name.trim();
  if (!trimmed) {
...
export async function deleteScheduledJob(job: ScheduledJob, jobFile: string): Promise<void> {
  // Allow deletion on all platforms
