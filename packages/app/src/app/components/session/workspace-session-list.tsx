    ...
    <>
              ...
              <div class="space-y-2">
                <div class="relative group">
                  <div
                    role="button"
                    tabIndex={0}
                    class="w-full flex items-center justify-between min-h-11 px-3 rounded-xl text-left transition-colors text-black hover:bg-gray-50"
                    onClick={() => {
                  ...
                  >
                    <button
                      type="button"
                      class="mr-2 -ml-1 p-1 rounded-md text-gray-600 hover:text-black hover:bg-gray-100"
                    ...
                    <Show when={group.status === "loading"}>
                      <Loader2 size={14} class="animate-spin text-gray-600 mr-1" />
                    </Show>
                    ...
                    <Show when={isConnecting()}>
                      <Loader2 size={14} class="animate-spin text-gray-600" />
                    </Show>
                  </div>
                  ...
                  <div class="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-0.5 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
                    <button
                      type="button"
                      class="p-1 rounded-md text-gray-600 hover:text-black hover:bg-gray-100"
                      onClick={(event) => {
                        event.stopPropagation();
                  ...
                  <Show when={isMenuOpen()}>
                    <div
                      ...
                      class="absolute right-2 top-[calc(100%+4px)] z-20 w-44 rounded-lg border border-gray-200 bg-white shadow-lg p-1"
                      onClick={(event) => event.stopPropagation()}
                    >
                      <button
                        type="button"
                        class="w-full text-left px-2 py-1.5 text-sm rounded-md hover:bg-gray-50"
                        onClick={() => {
                ...
                <div class="mt-0.5 space-y-0.5 border-l border-gray-6 ml-2">
                            ...
                            return (
                              <div
                                ...
                                class={`group flex items-center justify-between min-h-9 px-3 rounded-lg cursor-pointer relative overflow-hidden ml-2 w-[calc(100%-0.5rem)] ${
                                  isSelected() ? "bg-gray-100 text-black" : "hover:bg-gray-50"
                                }`}
                              ...
                              >
                                <div class="flex min-w-0 items-center gap-1.5 mr-2">
                                  ...
                                  <span class="text-[13px] text-black truncate font-medium">{session.title}</span>
                                </div>
                                <Show when={session.time?.updated}>
                                  <span class="text-[11px] text-gray-600 whitespace-nowrap group-hover:text-gray-700 transition-colors">
                  ...
                  >
                      ...
                      fallback={
                        ...
                        >
                          ...
                          <Show when={group.sessions.length === 0 && group.status === "ready"}>
                            <button
                              type="button"
                              class="group/empty w-full px-3 py-2 text-xs text-gray-600 ml-2 text-left rounded-lg hover:bg-gray-50 hover:text-black transition-colors"
                          ...
                          <Show when={group.sessions.length > previewCount(workspace().id)}>
                            <button
                              type="button"
                              class="ml-2 w-[calc(100%-0.5rem)] px-3 py-2 text-xs text-gray-600 hover:text-black hover:bg-gray-50 rounded-lg transition-colors text-left"
                    ...
                    >
                      <div class="w-full px-3 py-2 text-xs text-gray-600 ml-2 text-left rounded-lg">
                        Loading tasks...
                      </div>
                    </Show>
                  </Show>
      ...
      <div class="relative" ref={(el) => (addWorkspaceMenuRef = el)}>
        ...
        <Show when={addWorkspaceMenuOpen()}>
          <div class="absolute left-0 right-0 top-full mt-2 rounded-lg border border-gray-300 bg-white shadow-xl overflow-hidden z-20">
            <button
              type="button"
              class="w-full flex items-center gap-2 px-3 py-2 text-xs text-black hover:text-black hover:bg-gray-50 transition-colors"
              onClick={() => {
