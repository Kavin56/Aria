...
export function createWorkspaceStore(options: {
  ...
  async function bootstrapOnboarding() {
    ...
    // Auto-connect to backend if it's already running (even without a workspace)
    if (isTauriRuntime()) {
      const baseUrl = info?.baseUrl || "http://127.0.0.1:4096";
      
      // First, try to connect if engine info says it's running
      if (info?.running && info.baseUrl) {
        options.setStartupPreference("local");
        options.setOnboardingStep("connecting");
        const ok = await connectToServer(
      ...
      // Try connecting to default URL even if engine info says it's not running
      // (engine info might be stale, or backend might be running externally)
      if (!info?.running) {
        options.setStartupPreference("local");
        options.setOnboardingStep("connecting");
        const ok = await connectToServer(
          baseUrl,
          undefined,
          { reason: "bootstrap-fallback-connect" },
          engineAuth() ?? undefined,
        );
        if (ok) {
          markOnboardingComplete();
      ...
      // If connection failed and we have a project directory, try to start the backend
      if (!info?.running && projectDir().trim()) {
        options.setStartupPreference("local");
        options.setOnboardingStep("connecting");
        const ok = await startHost({ workspacePath: projectDir().trim() });
        if (ok) {
          markOnboardingComplete();
          return;
        }
      ...
      // Last resort: try to start with home directory as default workspace
      if (!info?.running && !projectDir().trim()) {
        try {
          const defaultDir = (await homeDir()).replace(/[\\/]+$/, "");
          if (defaultDir) {
            options.setStartupPreference("local");
            options.setOnboardingStep("connecting");
            const ok = await startHost({ workspacePath: defaultDir });
            if (ok) {
              markOnboardingComplete();
              return;
            }
          ...
          // Ignore errors, fall through to welcome screen
        }
      }
    }
