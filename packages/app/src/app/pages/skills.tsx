...
export default function SkillsView(props: SkillsViewProps) {
  ...
  return (
    <section class="space-y-8">
      <Show when={toast()}>
        <div class="fixed bottom-6 right-6 z-50 max-w-sm rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-black dark:bg-gray-800 px-4 py-3 text-xs text-black dark:text-white dark:text-white shadow-2xl">
          {toast()}
        </div>
      </Show>

      <div class="space-y-2">
        <h2 class="text-3xl font-bold text-black dark:text-white dark:text-white">{translate("skills.title")}</h2>
        <p class="text-sm text-gray-600 dark:text-gray-400">{translate("skills.subtitle")}</p>
      </div>

      <div class="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-black dark:bg-gray-800 px-5 py-5 shadow-sm">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div class="min-w-0 space-y-1">
            <div class="text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-600 dark:text-gray-400">Worker profile</div>
            <div class="text-xl font-semibold text-black dark:text-white dark:text-white truncate">{workspaceLabel()}</div>
          ...
          <button
            ...
            class={`inline-flex items-center gap-1.5 rounded-lg px-4 py-2 text-xs font-medium transition-colors ${
              canCreateInChat()
                ? "bg-black dark:bg-gray-800 text-white dark:text-white hover:opacity-90"
        ...
        <div class="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
          <div class="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-black dark:bg-gray-800 px-3 py-2.5">
            <div class="text-[11px] text-gray-600 dark:text-gray-400">Installed</div>
            <div class="mt-1 text-base font-semibold text-black dark:text-white dark:text-white">{props.skills.length}</div>
          </div>
          <div class="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-black dark:bg-gray-800 px-3 py-2.5">
            <div class="text-[11px] text-gray-600 dark:text-gray-400">Hub available</div>
            <div class="mt-1 text-base font-semibold text-black dark:text-white dark:text-white">{availableHubSkills().length}</div>
          </div>
          <div class="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-black dark:bg-gray-800 px-3 py-2.5">
            <div class="text-[11px] text-gray-600 dark:text-gray-400">Skill creator</div>
            <div class="mt-1 text-base font-semibold text-black dark:text-white dark:text-white">
              {skillCreatorInstalled() ? "Installed" : "Not installed"}
            </div>
          </div>
          <div class="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-black dark:bg-gray-800 px-3 py-2.5">
            <div class="text-[11px] text-gray-600 dark:text-gray-400">Mode</div>
            <div class="mt-1 text-base font-semibold text-black dark:text-white dark:text-white">
      ...
      <div class="flex flex-wrap items-center gap-3 border-b border-gray-200 dark:border-gray-800 pb-4">
        <button
          ...
          class={`flex items-center gap-1.5 text-xs font-medium transition-colors ${
            props.busy
              ...
              : "text-gray-600 dark:text-gray-400 hover:text-black dark:text-white dark:text-white"
          }`}
        >
        ...
        <div class="relative">
          <Search size={14} class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600 dark:text-gray-400" />
          <input
            type="text"
            value={searchQuery()}
            onInput={(event) => setSearchQuery(event.currentTarget.value)}
            placeholder="Search installed or hub skills"
            class="bg-white dark:bg-black dark:bg-gray-800 border border-gray-200 dark:border-gray-800 rounded-lg py-1.5 pl-9 pr-4 text-xs w-56 focus:w-72 focus:outline-none transition-all"
          />
        </div>
        <button
          type="button"
        ...
        <button
          ...
          class={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg transition-colors border ${
            props.busy
              ...
              : "border-gray-200 dark:border-gray-800 bg-white dark:bg-black dark:bg-gray-800 text-black dark:text-white dark:text-white hover:bg-gray-100 dark:bg-gray-900"
          }`}
      ...
      <Show when={props.accessHint}>
        <div class="text-xs text-gray-600 dark:text-gray-400">{props.accessHint}</div>
      </Show>
      <Show
        when={!props.accessHint && !props.canInstallSkillCreator && !props.canUseDesktopTools}
      >
        <div class="text-xs text-gray-600 dark:text-gray-400">{translate("skills.host_mode_only")}</div>
      </Show>

      <Show when={props.skillsStatus}>
        <div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 px-4 py-3 text-xs text-gray-600 dark:text-gray-400 whitespace-pre-wrap break-words">
          {props.skillsStatus}
        </div>
      </Show>

      <div class="space-y-4">
        <h3 class="text-[11px] font-bold text-gray-600 dark:text-gray-400 uppercase tracking-widest">
          {translate("skills.installed")}
        </h3>
        <Show
          when={filteredSkills().length}
          fallback={
            <div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-black dark:bg-gray-800 px-5 py-6 text-sm text-gray-600 dark:text-gray-400">
            ...
            <For each={filteredSkills()}>
              {(skill) => (
                <div
                  role="button"
                  tabindex="0"
                  class="bg-white dark:bg-black dark:bg-gray-800 border border-gray-200 dark:border-gray-800 rounded-xl p-4 flex items-start justify-between group hover:border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:bg-gray-900 transition-all text-left cursor-pointer"
                ...
                >
                  <div class="flex gap-4 min-w-0">
                    <div class="w-10 h-10 rounded-lg flex items-center justify-center shadow-sm border border-gray-200 dark:border-gray-800 bg-white dark:bg-black dark:bg-gray-800">
                      <Package size={20} class="text-gray-600 dark:text-gray-400" />
                    </div>
                    <div class="min-w-0">
                      <div class="flex items-center gap-2 mb-0.5">
                        <h4 class="text-sm font-semibold text-black dark:text-white dark:text-white truncate">{skill.name}</h4>
                        <Show when={isOpenworkInjectedSkill(skill)}>
                          <span class="rounded-full border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-400">
                            OpenWork
                          </span>
                        </Show>
                      </div>
                      ...
                      <div class="mt-1 text-[11px] font-mono text-gray-600 dark:text-gray-400 truncate">{skill.path}</div>
                    </div>
                  </div>
                  <div class="flex items-center gap-1">
                    <button
                      type="button"
                      class="p-1.5 text-gray-600 dark:text-gray-400 hover:text-black dark:text-white dark:text-white hover:bg-gray-100 dark:bg-gray-900 rounded-md transition-colors"
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        openShareLink(skill);
                    ...
                    <button
                      ...
                      class={`p-1.5 rounded-md transition-colors ${
                        props.busy || !props.canUseDesktopTools
                          ? "text-gray-600 dark:text-gray-400 opacity-40"
                          : "text-gray-600 dark:text-gray-400 hover:text-red-11 hover:bg-red-3/10"
                      }`}
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between gap-3">
          <h3 class="text-[11px] font-bold text-gray-600 dark:text-gray-400 uppercase tracking-widest">Install skills</h3>
          <button
            type="button"
        ...
        >
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <For each={filteredHubSkills()}>
              {(skill) => (
                <div class="bg-white dark:bg-black dark:bg-gray-800 border border-gray-200 dark:border-gray-800 rounded-xl p-4 flex items-start justify-between gap-4 group hover:border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:bg-gray-900 transition-all text-left">
                  <div class="flex gap-4 min-w-0">
                    <div class="w-10 h-10 rounded-lg flex items-center justify-center shadow-sm border border-gray-200 dark:border-gray-800 bg-white dark:bg-black dark:bg-gray-800">
                      <Package size={20} class="text-gray-600 dark:text-gray-400" />
                    </div>
                    <div class="min-w-0">
                      ...
                      <Show when={skill.description} fallback={<p class="text-xs text-gray-600 dark:text-gray-400">From openwork-hub</p>}>
                        <p class="text-xs text-gray-600 dark:text-gray-400 line-clamp-2">{skill.description}</p>
                      </Show>
                      <div class="mt-2 flex flex-wrap items-center gap-2 text-[11px] text-gray-600 dark:text-gray-400">
                        <span class="rounded-md border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 px-2 py-1 font-mono">
                          {skill.source.owner}/{skill.source.repo}
                        </span>
                        <Show when={skill.trigger}>
                          <span
                            class="inline-block max-w-full rounded-md border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 px-2 py-1 truncate"
      ...
      <div class="space-y-4">
        <h3 class="text-[11px] font-bold text-gray-600 dark:text-gray-400 uppercase tracking-widest">Capability setup</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <For each={recommendedSkills()}>
            {(item) => (
              <div
                role="button"
                tabindex="0"
                class={`bg-white dark:bg-black dark:bg-gray-800 border border-gray-200 dark:border-gray-800 rounded-xl p-4 flex items-start justify-between group transition-all text-left ${
                  item.disabled ? "opacity-80" : "hover:border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:bg-gray-900"
                }`}
                onClick={() => {
                  if (item.disabled) {
              ...
              >
                <div class="flex gap-4 min-w-0">
                  <div class="w-10 h-10 rounded-lg flex items-center justify-center shadow-sm border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900">
                    <item.icon size={20} class="text-gray-600 dark:text-gray-400" />
                  </div>
                  <div class="min-w-0">
                    <div class="flex items-center gap-2 mb-0.5">
                      <h4 class="text-sm font-semibold text-black dark:text-white dark:text-white truncate">{item.title}</h4>
                    </div>
                    <p class="text-xs text-gray-600 dark:text-gray-400 line-clamp-2">{item.description}</p>
                ...
                <button
                  ...
                  class={`p-1.5 rounded-md transition-colors ${
                    item.disabled
                      ? "text-gray-600 dark:text-gray-400 opacity-40"
                      : "text-gray-600 dark:text-gray-400 hover:text-black dark:text-white dark:text-white hover:bg-gray-50 dark:bg-gray-900"
                  }`}
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
      ...
      <Show when={selectedSkill()}>
        <div class="fixed inset-0 z-50 bg-black dark:bg-gray-800/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div class="w-full max-w-4xl rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-black dark:bg-gray-800 shadow-2xl overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between gap-3">
              <div class="min-w-0">
                <div class="text-sm font-semibold text-black dark:text-white dark:text-white truncate">{selectedSkill()!.name}</div>
                <div class="text-xs text-gray-600 dark:text-gray-400 truncate">{selectedSkill()!.path}</div>
              </div>
              <div class="flex items-center gap-2">
                <button
                  ...
                  class={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${
                    selectedDirty() && !props.busy
                      ? "bg-black dark:bg-gray-800 text-dls-surface hover:opacity-90"
                      : "bg-gray-100 dark:bg-gray-900 text-gray-600 dark:text-gray-400"
                  }`}
                ...
                <button
                  type="button"
                  class="px-3 py-1.5 text-xs font-medium rounded-lg bg-gray-50 dark:bg-gray-900 text-black dark:text-white dark:text-white hover:bg-gray-100 dark:bg-gray-900 transition-colors"
                  onClick={closeSkill}
                >
                  Close
                </button>
            ...
            <div class="p-5">
              ...
              <Show
                ...
                fallback={<div class="text-xs text-gray-600 dark:text-gray-400">Loading…</div>}
              >
                <textarea
                  ...
                  class="w-full min-h-[420px] rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 px-4 py-3 text-xs font-mono text-black dark:text-white dark:text-white focus:outline-none focus:ring-2 focus:ring-[rgba(var(--dls-accent-rgb)/0.25)]"
                  spellcheck={false}
                />
              </Show>
            </div>
      ...
      <Show when={uninstallOpen()}>
        <div class="fixed inset-0 z-50 bg-black dark:bg-gray-800/20 backdrop-blur-sm flex items-center justify-center p-4">
          <div class="bg-white dark:bg-black dark:bg-gray-800 border border-gray-200 dark:border-gray-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
            <div class="p-6">
              <div class="flex items-start justify-between gap-4">
                <div>
                  <h3 class="text-lg font-semibold text-black dark:text-white dark:text-white">{translate("skills.uninstall_title")}</h3>
              ...
              <div class="mt-4 rounded-xl bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-3 text-xs text-gray-600 dark:text-gray-400 font-mono break-all">
        ...
        <div class="fixed inset-0 z-50 bg-black dark:bg-gray-800/20 backdrop-blur-sm flex items-center justify-center p-4">
          <div class="bg-white dark:bg-black dark:bg-gray-800 border border-gray-200 dark:border-gray-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
            <div class="p-6 space-y-4">
              <div>
                <h3 class="text-lg font-semibold text-black dark:text-white dark:text-white">Share link</h3>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  Publish a public link. Anyone with the URL can install this skill.
                </p>
              </div>

              <div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 px-4 py-3 text-xs text-gray-600 dark:text-gray-400">
                <div class="font-semibold text-black dark:text-white dark:text-white">{shareTarget()?.name}</div>
              ...
              >
                <div class="rounded-xl bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-3 text-xs text-gray-600 dark:text-gray-400 font-mono break-all">
                  {shareUrl()}
                </div>
                <div class="flex justify-end gap-2">
                  <Button
      ...
      <Show when={installLinkOpen()}>
        <div class="fixed inset-0 z-50 bg-black dark:bg-gray-800/20 backdrop-blur-sm flex items-center justify-center p-4">
          <div class="bg-white dark:bg-black dark:bg-gray-800 border border-gray-200 dark:border-gray-800 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
            <div class="p-6 space-y-4">
              <div>
                <h3 class="text-lg font-semibold text-black dark:text-white dark:text-white">Install from link</h3>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Paste a skill bundle URL, preview it, then install.</p>
              </div>

              <div class="space-y-2">
                <div class="text-xs font-semibold uppercase tracking-widest text-gray-600 dark:text-gray-400">Link</div>
                <input
                  type="url"
                  value={installLinkUrl()}
                  onInput={(e) => setInstallLinkUrl(e.currentTarget.value)}
                  placeholder="https://share.openwork.software/b/..."
                  class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg px-3 py-2 text-xs font-mono text-black dark:text-white dark:text-white focus:outline-none"
                  spellcheck={false}
                />
              </div>
              ...
              <Show when={installLinkBundle()}>
                {(bundle) => {
                  ...
                  return (
                    <div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 p-4 space-y-2">
                      <div class="text-xs font-semibold text-black dark:text-white dark:text-white">Preview</div>
                      <div class="text-xs text-gray-600 dark:text-gray-400">
                        Skill: <span class="font-mono">{bundle().name}</span>
                      </div>
                      <Show when={bundle().description}>
                        <div class="text-xs text-gray-600 dark:text-gray-400">{bundle().description}</div>
                      </Show>
                      <Show when={conflict}>
