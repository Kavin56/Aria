  ...
  return (
    <section class="space-y-8 animate-in fade-in duration-300">
      ...
      <Show when={showHeader()}>
        <div>
          <h2 class="text-3xl font-bold text-black dark:text-white">{tr("mcp.apps_title")}</h2>
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between">
          ...
          <span class="text-[11px] text-gray-600 dark:text-gray-400">{tr("mcp.one_click_connect")}</span>
        </div>
                ...
                <button
                  ...
                  class={`group text-left rounded-xl border p-4 transition-all ${
                    connected()
                      ...
                      : "border-gray-200 dark:border-gray-800 bg-white dark:bg-black hover:bg-gray-50 dark:bg-gray-900 hover:shadow-[0_4px_16px_rgba(17,24,39,0.06)]"
                  }`}
                >
                  <div class="flex items-start gap-3">
                    ...
                    <div class={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 border ${
                      ...
                      <Show
                        when={!connecting()}
                        fallback={<Loader2 size={18} class="animate-spin text-gray-600 dark:text-gray-400" />}
                      >
                        <Show
                          when={!connected()}
                    ...
                    <div class="min-w-0 flex-1">
                      <div class="flex items-center gap-2">
                        <h4 class="text-sm font-semibold text-black dark:text-white">{entry.name}</h4>
                        <Show when={connected()}>
      ...
      <div class="space-y-4">
        <div class="flex flex-wrap items-center justify-between gap-3">
          ...
          <Show when={props.mcpLastUpdatedAt}>
            <span class="text-[11px] text-gray-600 dark:text-gray-400 tabular-nums">
        ...
        <Show
          ...
          fallback={
            <div class="rounded-xl border border-dashed border-gray-200 dark:border-gray-800 px-5 py-10 text-center">
              <Unplug size={24} class="mx-auto text-gray-600 dark:text-gray-400/30 mb-3" />
              <div class="text-sm font-medium text-gray-600 dark:text-gray-400">{tr("mcp.no_apps_yet")}</div>
              <div class="text-xs text-gray-600 dark:text-gray-400/60 mt-1">{tr("mcp.no_apps_hint")}</div>
            </div>
          }
        >
          <div class="space-y-2">
                ...
                return (
                  <div class={`rounded-xl border transition-all ${
                    isSelected()
                      ...
                      : "border-gray-200 dark:border-gray-800 bg-white dark:bg-black hover:bg-gray-50 dark:bg-gray-900"
                  }`}>
                    {/* Clickable row */}
                    <button
                      type="button"
                    ...
                    >
                      <div class="flex items-center gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                          <div class="text-sm font-medium text-black dark:text-white truncate">{entry.name}</div>
                        </div>
                        ...
                        <div class={`transition-transform ${isSelected() ? "rotate-180" : ""}`}>
                          <ChevronDown size={14} class="text-gray-600 dark:text-gray-400/40" />
                        </div>
                      </div>
                    </button>
                    ...
                    <Show when={isSelected()}>
                      <div class="border-t border-blue-6/20 px-4 py-3 space-y-3 animate-in fade-in slide-in-from-top-1 duration-200">
                        ...
                        <div class="flex items-center gap-4 text-xs">
                          <span class="text-gray-600 dark:text-gray-400">{tr("mcp.connection_type")}</span>
                        ...
                        {/* Capabilities */}
                        <div class="flex items-center gap-2">
                          <span class="text-[10px] font-medium bg-white dark:bg-black text-black dark:text-white border border-gray-200 dark:border-gray-800 px-2 py-0.5 rounded-md">
                            {tr("mcp.cap_tools")}
                          </span>
                        ...
                        {/* Technical details */}
                        <details class="group">
                          <summary class="flex items-center gap-1.5 text-[11px] text-gray-600 dark:text-gray-400 cursor-pointer hover:text-black dark:text-white transition-colors list-none">
                            <Code2 size={11} />
                            {tr("mcp.technical_details")}
                            <ChevronDown size={10} class="group-open:rotate-180 transition-transform" />
                          </summary>
                          <div class="mt-1.5 rounded-lg bg-gray-50 dark:bg-gray-900 px-3 py-2 text-[11px] font-mono text-gray-600 dark:text-gray-400 break-all">
                        ...
                        <Show when={supportsOauth(entry) && status() !== "connected"}>
                          ...
                          <div class="text-[11px] text-gray-600 dark:text-gray-400/70">
                            {tr("mcp.login_hint")}
                          </div>
                        </Show>
      ...
      <div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-black overflow-hidden">
        <button
          type="button"
          class="w-full flex items-center justify-between px-5 py-4 hover:bg-gray-50 dark:bg-gray-900 transition-colors"
          onClick={() => setShowAdvanced(!showAdvanced())}
        >
          <div class="flex items-center gap-3">
            <Settings2 size={16} class="text-gray-600 dark:text-gray-400" />
            <div class="text-left">
              <div class="text-sm font-medium text-black dark:text-white">{tr("mcp.advanced_settings")}</div>
              <div class="text-xs text-gray-600 dark:text-gray-400">{tr("mcp.advanced_settings_hint")}</div>
            </div>
          </div>
        ...
        <Show when={showAdvanced()}>
          <div class="border-t border-gray-200 dark:border-gray-800 px-5 py-4 space-y-4 animate-in fade-in slide-in-from-top-1 duration-200">
            {/* Scope toggle */}
              ...
              <button
                class={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                  configScope() === "project"
                    ...
                    : "text-gray-600 dark:text-gray-400 hover:text-black dark:text-white hover:bg-gray-50 dark:bg-gray-900"
                }`}
            ...
            <div class="flex flex-col gap-1 text-xs">
              <div class="text-gray-600 dark:text-gray-400">{tr("mcp.config_file")}</div>
              <div class="text-gray-600 dark:text-gray-400/80 font-mono text-[11px] truncate">
            ...
            <div class="flex items-center justify-between gap-3">
              <div class="flex items-center gap-2">
                ...
                <a
                  ...
                  class="inline-flex items-center gap-1 text-xs text-gray-600 dark:text-gray-400 hover:text-black dark:text-white transition-colors"
                >
                  {tr("mcp.docs_link")}
              ...
              <Show when={activeConfig() && activeConfig()!.exists === false}>
                <div class="text-[11px] text-gray-600 dark:text-gray-400">{tr("mcp.file_not_found")}</div>
              </Show>
            </div>
