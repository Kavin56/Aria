...
const automationTemplates = [
  ...
  },
  {
    ...
    description: "Check in on personal wellness and self-care routines.",
    prompt: "Remind me to take breaks, hydrate, and check in on my wellness goals. Suggest one small action I can take right now.",
    tone: "text-rose-9",
    scheduleMode: "daily" as const,
    scheduleTime: "14:00",
    scheduleDays: ["mo", "tu", "we", "th", "fr"],
  },
  {
    ...
    description: "Daily moment for reflection and gratitude practice.",
    prompt: "Help me reflect on today's experiences, identify moments of gratitude, and set a positive intention for tomorrow.",
    tone: "text-purple-9",
    scheduleMode: "daily" as const,
    scheduleTime: "20:00",
    scheduleDays: ["mo", "tu", "we", "th", "fr", "sa", "su"],
  },
];
...
const AutomationCard = (props: {
  ...
  return (
    <button
      ...
      class={`group w-full rounded-2xl border bg-white dark:bg-black p-5 text-left transition-shadow hover:shadow-md ${
        props.disabled ? "cursor-not-allowed opacity-60" : "cursor-pointer"
      } border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700`}
    >
      <div class={`mb-4 flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-black`}>
        <Icon size={18} class={props.tone ?? "text-gray-600 dark:text-gray-400"} />
      </div>
      <p class="text-[13px] text-gray-600 dark:text-gray-300 leading-relaxed group-hover:text-black dark:group-hover:text-white">{props.description}</p>
    </button>
  );
};
...
const AutomationJobCard = (props: {
  ...
  return (
    <div class="flex flex-col gap-4 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-black p-5 shadow-sm">
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="flex min-w-0 items-start gap-3">
          <div
            class={`flex h-8 w-8 items-center justify-center rounded-lg border bg-white dark:bg-black ${statusIconTone(
              status()
            )}`}
          >
            <Calendar size={18} class="text-gray-600 dark:text-gray-400" />
          </div>
          <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-2">
              <h3 class="text-sm font-semibold text-black dark:text-white truncate">{props.job.name}</h3>
              <span
            ...
            <div class="mt-1 text-xs text-gray-600 dark:text-gray-300">{scheduleLabel()}</div>
          </div>
        </div>
        <div class="flex items-center gap-2">
          <button
            ...
            class={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors ${
              !props.supported || props.busy
                ...
                : "border-gray-300 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-900 hover:text-black dark:hover:text-white"
            }`}
          >
            <Play size={12} />
            Run
          ...
          <button
            ...
            class={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors ${
              !props.supported || props.busy
                ? "border-gray-300 dark:border-gray-700 text-gray-500 dark:text-gray-500"
                : "border-red-6 dark:border-red-700 text-red-10 dark:text-red-400 hover:bg-red-3 dark:hover:bg-red-900"
            }`}
          >
            <Trash2 size={12} />
            Delete
      ...
      <div class="grid gap-3 md:grid-cols-2">
        <div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-black px-3 py-3">
          <div class="text-[10px] uppercase tracking-wide text-gray-500 dark:text-gray-400">{summary().label}</div>
          <div
            class={`mt-1 text-sm text-black dark:text-white break-words ${summary().mono ? "font-mono" : ""}`}
          >
            {summary().value}
          </div>
        </div>
        <div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-black px-3 py-3 space-y-2">
          <div class="text-[10px] uppercase tracking-wide text-gray-500 dark:text-gray-400">Run context</div>
          <div class="space-y-2 text-xs text-gray-600 dark:text-gray-300">
            <div class="flex items-center gap-2">
              <FolderOpen size={14} class="text-gray-500 dark:text-gray-400" />
              <span class="font-mono text-black dark:text-white break-all">
            ...
            <Show when={props.job.source}>
              <div class="text-[11px] text-gray-500 dark:text-gray-400">Source: {props.job.source}</div>
            </Show>
          </div>
        </div>
      </div>
...
export default function ScheduledTasksView(props: ScheduledTasksViewProps) {
  ...
  const supportNote = createMemo(() => {
    if (props.source === "remote") {
      return props.sourceReady ? null : "SHIFT server unavailable. Connect to sync scheduled tasks.";
    }
  ...
  const sourceDescription = createMemo(() =>
    props.source === "remote"
      ? "Automations that run on a schedule from the connected SHIFT server."
      : "Automations that run on a schedule from this device."
  );
  const sourceLabel = createMemo(() =>
    props.source === "remote" ? "From SHIFT server" : "From local scheduler"
  );
  const schedulerLabel = createMemo(() => (props.source === "remote" ? "SHIFT server" : "Local"));
  ...
  const schedulerUnavailableHint = createMemo(() =>
    props.source === "remote" ? "SHIFT server unavailable" : "Desktop-only"
  );
  const deleteDescription = createMemo(() =>
    props.source === "remote"
      ? "This removes the schedule and deletes the job definition from the connected SHIFT server."
    ...
    <section class="space-y-10">
      <div class="flex flex-wrap items-center justify-end gap-4">
        <button
          ...
          class="text-xs font-medium text-gray-600 dark:text-gray-300 transition-colors hover:text-black dark:hover:text-white"
        >
          Learn more
        </button>
        <button
      ...
      <div class="pt-8 text-center">
        <div class="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-black shadow-sm">
          <Terminal size={28} class="text-gray-600 dark:text-gray-400" />
        </div>
        <div class="flex items-center justify-center gap-2">
          <h2 class="text-2xl font-semibold text-black dark:text-white">Automations</h2>
          <span class="rounded border border-gray-200 dark:border-gray-800 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-tight text-gray-500 dark:text-gray-400">
            Beta
          </span>
        </div>
        <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">{sourceDescription()}</p>
      </div>

      <Show when={schedulerGateActive()}>
        <div class="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-black px-5 py-5 shadow-sm">
          <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="flex items-start gap-3">
              <div class="flex h-10 w-10 items-center justify-center rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-black">
                <PlugZap size={18} class="text-gray-600 dark:text-gray-400" />
              </div>
              <div>
                <div class="text-sm font-semibold text-black dark:text-white">
                  {schedulerGateMode() === "reload"
                    ? "Reload SHIFT to activate automations"
                    : "Install the scheduler to unlock automations"}
                </div>
                <div class="mt-1 text-xs text-gray-600 dark:text-gray-300">
                  {schedulerGateMode() === "reload"
                    ? "OpenCode loads plugins at startup. Reload SHIFT to activate opencode-scheduler."
            ...
            <div class="flex flex-wrap items-center gap-2">
              ...
              >
                {props.reloadBusy ? "Reloading..." : "Reload SHIFT"}
              </Button>
              <button
                type="button"
                onClick={openSchedulerDocs}
      ...
      <Show when={supportNote()}>
        <div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-black px-5 py-4 text-sm text-gray-600 dark:text-gray-300">
          {supportNote()}
        </div>
      </Show>
        ...
        fallback={
          <div class={`space-y-4 ${schedulerGateActive() ? "opacity-60 pointer-events-none" : ""}`}>
            ...
            <button
              type="button"
              onClick={openSchedulerDocs}
              class="mx-auto block text-xs text-gray-600 dark:text-gray-300 transition-colors hover:text-black dark:hover:text-white"
            >
              Explore more
            </button>
          </div>
      ...
      <Show when={deleteTarget()}>
        <div class="fixed inset-0 z-50 bg-black/20 backdrop-blur-sm flex items-center justify-center p-4">
          <div class="bg-white dark:bg-black border border-gray-300 dark:border-gray-700 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
            <div class="p-6 space-y-4">
              <div class="flex items-start justify-between gap-4">
                <div>
                  <h3 class="text-lg font-semibold text-black dark:text-white">Delete automation?</h3>
                  <p class="text-sm text-gray-600 dark:text-gray-300 mt-1">{deleteDescription()}</p>
                </div>
              </div>
              <div class="rounded-xl bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 p-3 text-xs text-gray-600 dark:text-gray-300 font-mono break-all">
                {deleteTarget()?.name}
              </div>
      ...
      <Show when={createModalOpen()}>
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-[2px] p-4">
          <div class="w-full max-w-2xl rounded-3xl bg-white dark:bg-black shadow-2xl overflow-hidden border border-gray-300 dark:border-gray-700">
            <div class="p-8 space-y-6">
              <div class="flex items-start justify-between gap-4">
                <div>
                  <h2 class="text-xl font-semibold text-black dark:text-white">Create automation</h2>
                ...
                <button
                  ...
                  class="rounded-full p-1 text-gray-500 dark:text-gray-400 transition-colors hover:bg-gray-50 dark:hover:bg-gray-900 hover:text-black dark:hover:text-white"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <div class="space-y-6">
                <div>
                  ...
                  <input
                    ...
                    class="w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-black px-3 py-2 text-sm text-black dark:text-white focus:outline-none focus:ring-1 focus:ring-blue-500/20 focus:border-blue-500"
                  />
                </div>
                <div>
                  <label class="mb-2 block text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                ...
                <div>
                  ...
                  <div class="rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-black p-3">
                    <textarea
                      rows={4}
                      value={automationPrompt()}
                      onInput={(event) => setAutomationPrompt(event.currentTarget.value)}
                      class="w-full resize-none bg-transparent text-sm text-black dark:text-white focus:outline-none"
                    />
                  </div>
                </div>
                <div>
                  <div class="mb-2 flex items-center justify-between">
                    ...
                    <div class="flex rounded-lg bg-gray-3 dark:bg-gray-900 p-0.5">
                      <button
                        type="button"
                  ...
                  <Show
                    ...
                    fallback={
                      <div class="flex flex-wrap items-center gap-3">
                        <div class="flex items-center gap-2 rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm text-black">
                          <span>Every</span>
                          <input
                            type="number"
                            min={1}
                  ...
                  >
                    <div class="flex flex-wrap items-center gap-3">
                      <div class="flex items-center justify-between rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm text-black">
                        <input
                          type="time"
                          value={scheduleTime()}
