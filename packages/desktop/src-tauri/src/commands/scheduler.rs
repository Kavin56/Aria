...
fn require_scheduler_support() -> Result<(), String> {
    ...
    Err("Scheduler is supported on macOS, Linux, and Windows.".to_string())
}
...
fn uninstall_job(slug: &str, scope_id: Option<&str>) -> Result<(), String> {
    let task_name = if let Some(scope_id) = scope_id {
        format!("OpenCode_{}_{}", scope_id, slug)
    } else {
        format!("OpenCode_{}", slug)
    };
